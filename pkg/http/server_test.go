	...
	Context("NewServer", func() {
		...
		DescribeTable("When creating the new server from the options", func(in *newServerTableInput) {
			...
			Entry("with an invalid bind address port", &newServerTableInput{
				...
				expectedErr:        errors.New("error setting up listener: listen (tcp, 127.0.0.1:a) failed: listen tcp: lookup tcp/a: Servname not supported for ai_socktype"),
				expectHTTPListener: false,
				expectTLSListener:  false,
			}),
			Entry("with an invalid secure bind address port", &newServerTableInput{
				...
				expectedErr:        errors.New("error setting up TLS listener: listen (127.0.0.1:a) failed: listen tcp: lookup tcp/a: Servname not supported for ai_socktype"),
				expectHTTPListener: false,
				expectTLSListener:  false,
			}),
